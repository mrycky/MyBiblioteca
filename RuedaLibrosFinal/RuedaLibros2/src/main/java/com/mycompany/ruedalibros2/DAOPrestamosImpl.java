...
public class DAOPrestamosImpl extends Conexion implements DAOPrestamos{
    ...
    public void registrar(Prestamo user) throws Exception {
        try {
            this.Conectar();
            PreparedStatement st = this.conexion.prepareStatement("INSERT INTO prestamo(id_usuario, id_libro, fecha_salida) VALUES(?,?,?);");
            st.setInt(1, user.getId_usuario());
            st.setInt(2, user.getId_libro());
            st.setString(3, user.getFecha_salida());
            st.executeUpdate();
            st.close();
        } catch(Exception e) {
            throw e;
    ...
    @Override
    public void modificar(Prestamo prestamos) throws Exception {
        try {
            this.Conectar();
            ...
            st.setInt(1, prestamos.getId_usuario());
            st.setInt(2, prestamos.getId_libro());
            st.setString(3, prestamos.getFecha_salida());
            st.setString(4, prestamos.getFecha_regreso());
            st.setInt(5, prestamos.getId());
            st.executeUpdate();
            st.close();
        } catch(Exception e) {
            throw e;
